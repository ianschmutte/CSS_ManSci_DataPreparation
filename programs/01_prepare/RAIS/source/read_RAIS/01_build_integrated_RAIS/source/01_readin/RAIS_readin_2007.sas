/********************
	RAIS_readin_2007.sas
	Ian M. Schmutte
  2018 January 1
	DESCRIPTION: Read raw input files from original 2007 data
*********************/
...
%let states = centooeste07  nordeste07 norte07 sp07 sudeste07 sul07 ;

/* read-in macro */
%macro readin2007(st,file);
       filename raw pipe "7z e -so &raw_path./20171128_data/RAIS_anteriores/2007/&st..7z 2>> error.log";
            data INTERWRK.&st.2007;
  ...
  /*smell testing*/
  proc contents data=INTERWRK.&st.2007;
  run;

  proc print data=INTERWRK.&st.2007 (obs=50);
  run;

%mend;
/* run macro for all data in 2007 */
...
%macro runall_states;
  ...
  %do %while ("%scan(&states.,&i.)" ne "" ) ;
    ...
    %readin2007(&state.,&state.);
    %let i=%eval(&i.+1);
  %end;

  data INTERWRK.RAIS2007_raw;
    set
      %let i=1 ;
        %do %while ("%scan(&states.,&i.)" ne "" ) ;
          %let state=%scan(&states.,&i.);
...
%runall_states;
  ...
  proc contents data=INTERWRK.RAIS2007_raw;
  run;

  proc print data=INTERWRK.RAIS2007_raw (obs=100);
  run;
