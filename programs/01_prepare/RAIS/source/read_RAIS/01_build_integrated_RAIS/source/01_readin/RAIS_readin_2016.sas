/********************
	RAIS_readin_2016.sas
	Ian M. Schmutte
  2018 January 1
	DESCRIPTION: Read raw input files from original 2016 data
*********************/
...
/* read-in macro */
%macro readin2016(st,file);
       filename raw pipe "7z e -so &raw_path./20171128_data/2016/&st.2016ID.7z 2>> error.log";
            data INTERWRK.&st.2016 (drop = YEAR_OF_ARRIVAL_DUP); /*november is not in these data, but a duplicate of ano chegada. See header scans*/
  ...
  /*smell testing*/
  proc contents data=INTERWRK.&st.2016;
  run;

  proc print data=INTERWRK.&st.2016 (obs=50);
  run;
...
%macro runall_states;
  ...
  %do %while ("%scan(&states.,&i.)" ne "" ) ;
    ...
    %readin2016(&state.,&state.);
    %let i=%eval(&i.+1);
  %end;

  data INTERWRK.RAIS2016_raw;
    set
      %let i=1 ;
        %do %while ("%scan(&states.,&i.)" ne "" ) ;
          %let state=%scan(&states.,&i.);
...
%runall_states;
  ...
  proc contents data=INTERWRK.RAIS2016_raw;
  run;

  proc print data=INTERWRK.RAIS2016_raw (obs=100);
  run;
