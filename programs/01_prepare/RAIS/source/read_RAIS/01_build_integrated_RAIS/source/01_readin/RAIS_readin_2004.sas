/********************
	RAIS_readin_2004.sas
	Ian M. Schmutte
  2018 January 1
	DESCRIPTION: Read raw input files from original 2004 data
*********************/
...
%let states = centro_oeste2004 ES_MG_RJ2004 nordeste2004 Norte2004 SP2004 sul2004 ;

/* read-in macro */
%macro readin2004(st,file);
       filename raw pipe "7z e -so &raw_path./20171128_data/RAIS_anteriores/2004/&st..7z 2>> error.log";
            data INTERWRK.&st.2004;
  ...
  /*smell testing*/
  proc contents data=INTERWRK.&st.2004;
  run;

  proc print data=INTERWRK.&st.2004 (obs=50);
  run;

%mend;
/* run macro for all data in 2004 */
...
%macro runall_states;
  ...
  %do %while ("%scan(&states.,&i.)" ne "" ) ;
    ...
    %readin2004(&state.,&state.);
    %let i=%eval(&i.+1);
  %end;

  data INTERWRK.RAIS2004_raw;
    set
      %let i=1 ;
        %do %while ("%scan(&states.,&i.)" ne "" ) ;
          %let state=%scan(&states.,&i.);
...
%runall_states;
  ...
  proc contents data=INTERWRK.RAIS2004_raw;
  run;

  proc print data=INTERWRK.RAIS2004_raw (obs=100);
  run;
