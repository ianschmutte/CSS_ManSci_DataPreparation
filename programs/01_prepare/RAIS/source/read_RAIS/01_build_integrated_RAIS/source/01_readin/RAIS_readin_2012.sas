/********************
	RAIS_readin_2012.sas
	Ian M. Schmutte
  2018 January 1
	DESCRIPTION: Read raw input files from original 2012 data
*********************/
...
/* read-in macro */
%macro readin2012(st,file);
       filename raw pipe "7z e -so &raw_path./20171128_data/RAIS_anteriores/2012/&st.2012ID.7z 2>> error.log";
            data INTERWRK.&st.2012;
  ...
  /*smell testing*/
  proc contents data=INTERWRK.&st.2012;
  run;

  proc print data=INTERWRK.&st.2012 (obs=50);
  run;

%mend;
/* run macro for all data in 2012 */
...
%macro runall_states;
  ...
  %do %while ("%scan(&states.,&i.)" ne "" ) ;
    ...
    %readin2012(&state.,&state.);
    %let i=%eval(&i.+1);
  %end;

  data INTERWRK.RAIS2012_raw;
    set
      %let i=1 ;
        %do %while ("%scan(&states.,&i.)" ne "" ) ;
          %let state=%scan(&states.,&i.);
...
%runall_states;
  ...
  proc contents data=INTERWRK.RAIS2012_raw;
  run;

  proc print data=INTERWRK.RAIS2012_raw (obs=100);
  run;
