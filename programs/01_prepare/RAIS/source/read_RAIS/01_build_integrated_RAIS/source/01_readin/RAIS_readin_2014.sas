/********************
	RAIS_readin_2014.sas
	Ian M. Schmutte
  2018 January 1
	DESCRIPTION: Read raw input files from original 2014 data
*********************/
...
/* read-in macro */
%macro readin2014(st,file);
       filename raw pipe "7z e -so &raw_path./20171128_data/2014/&st.2014ID.7z 2>> error.log";
            data INTERWRK.&st.2014;
  ...
  /*smell testing*/
  proc contents data=INTERWRK.&st.2014;
  run;

  proc print data=INTERWRK.&st.2014 (obs=50);
  run;

%mend;
/* run macro for all data in 2014 */
...
%macro runall_states;
  ...
  %do %while ("%scan(&states.,&i.)" ne "" ) ;
    ...
    %readin2014(&state.,&state.);
    %let i=%eval(&i.+1);
  %end;

  data INTERWRK.RAIS2014_raw;
    set
      %let i=1 ;
        %do %while ("%scan(&states.,&i.)" ne "" ) ;
          %let state=%scan(&states.,&i.);
...
%runall_states;
  ...
  proc contents data=INTERWRK.RAIS2014_raw;
  run;

  proc print data=INTERWRK.RAIS2014_raw (obs=100);
  run;
