/********************
	RAIS_readin_2006.sas
	Ian M. Schmutte
  2018 January 1
	DESCRIPTION: Read raw input files from original 2006 data
*********************/
...
%let states = centro06  nordeste06 norte06 sp06 sp106 sudeste06 sul06 ;

/* read-in macro */
%macro readin2006(st,file);
       filename raw pipe "7z e -so &raw_path./20171128_data/RAIS_anteriores/2006/&st..7z 2>> error.log";
            data INTERWRK.&st.2006;
  ...
  /*smell testing*/
  proc contents data=INTERWRK.&st.2006;
  run;

  proc print data=INTERWRK.&st.2006 (obs=50);
  run;
...
%macro runall_states;
  ...
  %do %while ("%scan(&states.,&i.)" ne "" ) ;
    ...
    %readin2006(&state.,&state.);
    %let i=%eval(&i.+1);
  %end;

  data INTERWRK.RAIS2006_raw;
    set
      %let i=1 ;
        %do %while ("%scan(&states.,&i.)" ne "" ) ;
          %let state=%scan(&states.,&i.);
...
%runall_states;
  ...
  proc contents data=INTERWRK.RAIS2006_raw;
  run;

  proc print data=INTERWRK.RAIS2006_raw (obs=100);
  run;
