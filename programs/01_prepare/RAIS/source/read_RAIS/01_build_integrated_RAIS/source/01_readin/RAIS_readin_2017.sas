/********************
	RAIS_readin_2017.sas
	Ian M. Schmutte
  2019 January 16
	DESCRIPTION: Read raw input files from original 2017 data
*********************/
...
/* read-in macro */
%macro readin2017(st,file);
       filename raw pipe "7z e -so &raw_path./20181023_data/2017/&st.2017ID.7z 2>> error.log";
            data INTERWRK.&st.2017 (drop = YEAR_OF_ARRIVAL_DUP); /*this variable duplicated; see header rows*/ 
            ...
            informat IBGE_SUBSECTOR	           $15.  ;
            informat YEAR_OF_ARRIVAL	         $4.    ;
            informat CEP_OF_ESTAB	             $8.   ;
            informat WORKER_MUNI	             $6.    ;
            informat COMPANY_NAME	             $45.  ;
            informat JAN_PAY	                 numx13.;
            informat FEB_PAY	                 numx13.;
            informat MAR_PAY	                 numx13.;
            informat APR_PAY	                 numx13.;
            informat MAY_PAY	                 numx13.;
            informat JUN_PAY	                 numx13.;
            informat JUL_PAY	                 numx13.;
            informat AUG_PAY	                 numx13.;
            informat SEP_PAY	                 numx13.;
            informat OCT_PAY	                 numx13.;
            informat NOV_PAY	                 numx13.;
            informat YEAR_OF_ARRIVAL_DUP       $4.    ;
            informat PARTIAL_CONTRACT	         $1.    ;
            informat INTERMITTENT_CONTRACT	   $1.    ;
            informat WORKER_IN_UNION	         $1.    ;
          ...
          LABEL
            ...
            IBGE_SUBSECTOR = "IBGE Subsector"	        
            YEAR_OF_ARRIVAL	= "Year of arrival to Brazil (for non-natives)"      
            CEP_OF_ESTAB = "CEP code of establishment"	          
            WORKER_MUNI	= "Worker municipality of residence"          
            COMPANY_NAME = "Company name"	          
            JAN_PAY	= "Jan. compensation"              
            FEB_PAY	= "Feb. compensation"	              
            MAR_PAY	= "Mar. compensation"	              
            APR_PAY	= "Apr. compensation"	              
            MAY_PAY	= "May. compensation"	              
            JUN_PAY	= "Jun. compensation"	              
            JUL_PAY	= "Jul. compensation"	              
            AUG_PAY	= "Aug. compensation"	              
            SEP_PAY	= "Sep. compensation"	              
            OCT_PAY	= "Oct. compensation"	              
            NOV_PAY	= "Nov. compensation"	                    
            PARTIAL_CONTRACT = "Indicator for partial contract"	      
            INTERMITTENT_CONTRACT	= "Indicator for intermittent contract"
            WORKER_IN_UNION	= "Worker unionized"   
            ;   
  ...
  /*smell testing*/
  proc contents data=INTERWRK.&st.2017;
  run;

  proc print data=INTERWRK.&st.2017 (obs=10);
  run;

%mend;
/* run macro for all data in 2016 */
...
%macro runall_states;
  ...
  %do %while ("%scan(&states.,&i.)" ne "" ) ;
    ...
    %readin2017(&state.,&state.);
    %let i=%eval(&i.+1);
  %end;

  data INTERWRK.RAIS2017_raw;
    set
      %let i=1 ;
        %do %while ("%scan(&states.,&i.)" ne "" ) ;
          %let state=%scan(&states.,&i.);
...
%runall_states;
  ...
  proc contents data=INTERWRK.RAIS2017_raw;
  run;

  proc print data=INTERWRK.RAIS2017_raw (obs=100);
  run;
