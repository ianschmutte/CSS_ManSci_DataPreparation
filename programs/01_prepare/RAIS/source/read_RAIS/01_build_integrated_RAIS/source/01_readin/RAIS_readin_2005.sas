/********************
	RAIS_readin_2005.sas
	Ian M. Schmutte
  2018 January 1
	DESCRIPTION: Read raw input files from original 2005 data
*********************/
...
%let states = centro05  nordeste05 norte05 sp05 sp105 sudeste05 sul05 ;

/* read-in macro */
%macro readin2005(st,file);
       filename raw pipe "7z e -so &raw_path./20171128_data/RAIS_anteriores/2005/&st..7z 2>> error.log";
            data INTERWRK.&st.2005;
  ...
  /*smell testing*/
  proc contents data=INTERWRK.&st.2005;
  run;

  proc print data=INTERWRK.&st.2005 (obs=50);
  run;

%mend;
/* run macro for all data in 2006 */
...
%macro runall_states;
  ...
  %do %while ("%scan(&states.,&i.)" ne "" ) ;
    ...
    %readin2005(&state.,&state.);
    %let i=%eval(&i.+1);
  %end;

  data INTERWRK.RAIS2005_raw;
    set
      %let i=1 ;
        %do %while ("%scan(&states.,&i.)" ne "" ) ;
          %let state=%scan(&states.,&i.);
...
%runall_states;
  ...
  proc contents data=INTERWRK.RAIS2005_raw;
  run;

  proc print data=INTERWRK.RAIS2005_raw (obs=100);
  run;
