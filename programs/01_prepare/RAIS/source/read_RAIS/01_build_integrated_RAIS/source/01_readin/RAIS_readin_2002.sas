/********************
	RAIS_readin_2002.sas
	Ian M. Schmutte
  2018 January 1
	DESCRIPTION: Read raw input files from original 2002 data
*********************/
...
/* read-in macro */
%macro readin2002(st,file);
       filename raw pipe "7z e -so &raw_path./20171128_data/RAIS_anteriores/2002/&st.2002ID.7z 2>> error.log";
            data INTERWRK.&st.2002;
  ...
  /*smell testing*/
  proc contents data=INTERWRK.&st.2002;
  run;

  proc print data=INTERWRK.&st.2002 (obs=50);
  run;

%mend;
/* run macro for all data in 2002 */
...
%macro runall_states;
  ...
  %do %while ("%scan(&states.,&i.)" ne "" ) ;
    ...
    %readin2002(&state.,&state.);
    %let i=%eval(&i.+1);
  %end;

  data INTERWRK.RAIS2002_raw;
    set
      %let i=1 ;
        %do %while ("%scan(&states.,&i.)" ne "" ) ;
          %let state=%scan(&states.,&i.);
...
%runall_states;
  ...
  proc contents data=INTERWRK.RAIS2002_raw;
  run;

  proc print data=INTERWRK.RAIS2002_raw (obs=100);
  run;
