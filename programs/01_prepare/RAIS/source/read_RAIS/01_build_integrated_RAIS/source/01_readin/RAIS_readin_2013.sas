/********************
	RAIS_readin_2013.sas
	Ian M. Schmutte
  2018 January 1
	DESCRIPTION: Read raw input files from original 2013 data
*********************/
...
/* read-in macro */
%macro readin2013(st,file);
       filename raw pipe "7z e -so &raw_path./20171128_data/RAIS_anteriores/2013/&st.2013ID.7z 2>> error.log";
            data INTERWRK.&st.2013;
  ...
  /*smell testing*/
  proc contents data=INTERWRK.&st.2013;
  run;

  proc print data=INTERWRK.&st.2013 (obs=50);
  run;

%mend;
/* run macro for all data in 2013 */
...
%macro runall_states;
  ...
  %do %while ("%scan(&states.,&i.)" ne "" ) ;
    ...
    %readin2013(&state.,&state.);
    %let i=%eval(&i.+1);
  %end;

  data INTERWRK.RAIS2013_raw;
    set
      %let i=1 ;
        %do %while ("%scan(&states.,&i.)" ne "" ) ;
          %let state=%scan(&states.,&i.);
...
%runall_states;
  ...
  proc contents data=INTERWRK.RAIS2013_raw;
  run;

  proc print data=INTERWRK.RAIS2013_raw (obs=100);
  run;
