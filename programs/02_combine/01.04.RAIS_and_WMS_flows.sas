/********************
	DESCRIPTION: Get RAIS data for years 2008--2013 for WMS plants
*********************/
...
proc sort data = wms_list nodupkey out=wms_firms; 
  by cnpj;
run;
...
%macro bigstep;
data INTERWRK.RAIS_WMS_flows(keep = PIS cnpj year EARN_AVG_MONTH_REAL log_wage state CNAE20_CLASS occ_mngr occ_labr 										occ_dir occ_prod occ_supr occ_tech occ_wms_mngr hire sep fire CAUSE_OF_SEP 
									NUM_HOURS_CONTRACTED nohs somehs hs coll	educ_best TENURE_MONTHS age_31Dec male race_white race_pardo race_preto race_other has_akm_pe akm_pe akm_fe );
  merge PREPARED.RAIS_with_AKM (in=left where=(year ge 2008 and year le 2013))
        wms_firms (in = right keep = cnpj);
  by cnpj;
  if right then do; /*only keep if there's a match to the WMS*/
	...
	* occ_mngr = occ1 = "1";
    ...
    fire = 0;
    if CAUSE_OF_SEP = "11" then fire = 1;
	nohs = 0; 
	somehs = 0;
	hs=0; 
	coll=0;
...
proc contents data=INTERWRK.RAIS_WMS_flows;
run;

proc means data=INTERWRK.RAIS_WMS_flows nolabels;
	var _numeric_;
	class year;
run;


proc export data = INTERWRK.RAIS_WMS_flows outfile="&wrk_path./RAIS_WMS_flows.dta" DBMS=DTA replace;
run;
